...
const sendMessage = async (jid, content, options = {}) => {
  if (content?.edit) {
    ...
    // relayMessage() harus kamu definisikan juga
    return await relayMessage(jid, editedMsg, { messageId: content.edit.id })
  }

  throw new Error('sendMessage biasa belum didukung dalam versi ini.')
}

const relayMessage = async (jid, message, { messageId }) => {
  const deviceMessage = {
    deviceSentMessage: {
      destinationJid: jid,
      message
  ...
  await query({
    tag: 'message',
    attrs: { to: jid, id: messageId, type: 'chat' },
    content
  })
