...
const meId = authState.creds.me.id
  ...
  // PATCH: Deteksi pesan edit (protocolMessage.type === MESSAGE_EDIT)
  if (message?.protocolMessage?.type === WAProto.proto.Message.ProtocolMessage.Type.MESSAGE_EDIT) {
    message = {
      protocolMessage: {
        key: message.protocolMessage.key,
        type: WAProto.proto.Message.ProtocolMessage.Type.MESSAGE_EDIT,
        editedMessage: message.protocolMessage.editedMessage,
        timestampMs: Date.now()
      }
    }

    // Optional: Debug log
    logger?.info('📝 PATCH relayMessage: detected protocolMessage edit')

    // force kirim sebagai message dari bot sendiri
    const deviceMessage = {
      deviceSentMessage: {
        destinationJid: jid,
        message
    ...
    const content = await Utils_1.encodeWAMessage(deviceMessage)
    await query({
      tag: 'message',
      attrs: { to: jid, id: msgId, type: 'chat' },
      content
    })
...
message
  ...
  // PATCH EDIT MESSAGE SUPPORT
if (message?.protocolMessage?.type === WAProto_1.proto.Message.ProtocolMessage.Type.MESSAGE_EDIT) {
  const editKey = message.protocolMessage.key
  const edited = message.protocolMessage.editedMessage

  // rebuild deviceSentMessage payload
  message = {
    protocolMessage: {
  ...
  meMsg.deviceSentMessage.message = message
  logger.debug({ editKey }, '🚧 Edit message intercepted')
}
