...
if (message.edit) {
    const contentToEdit = m
    const editKey = message.edit

    const protocolMessage = {
        protocolMessage: {
            key: editKey,
            type: WAProto_1.proto.Message.ProtocolMessage.Type.MESSAGE_EDIT,
            editedMessage: contentToEdit,
            timestampMs: Date.now()
        }
    }

    return WAProto_1.proto.Message.fromObject(protocolMessage)
}
...
const generateWAMessageFromContent = (jid, message, options) => {
    ...
    const isEditMessage = message.protocolMessage && 
                         message.protocolMessage.type === WAProto_1.proto.Message.ProtocolMessage.Type.MESSAGE_EDIT
    ...
    // Use the same messageId for edit messages
    const messageId = isEditMessage ? message.protocolMessage.key.id : (options?.messageId || generics_1.generateMessageID())
